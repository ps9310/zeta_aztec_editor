        ...
        fun setUp(
            ...
            run {
                val channel = BasicMessageChannel<Any?>(
                    binaryMessenger,
                    "dev.flutter.pigeon.zeta_aztec_editor.AztecEditorApi.launch$separatedMessageChannelSuffix",
                    codec
                )
                if (api != null) {
...
class AztecFlutterApi(
    ...
    fun onAztecFileSelected(filePathArg: String, callback: (Result<String?>) -> Unit) {
        val separatedMessageChannelSuffix =
            if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
        val channelName =
            "dev.flutter.pigeon.zeta_aztec_editor.AztecFlutterApi.onAztecFileSelected$separatedMessageChannelSuffix"
    ...
    fun onAztecFileDeleted(filePathArg: String, callback: (Result<Unit>) -> Unit) {
        val separatedMessageChannelSuffix =
            if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
        val channelName =
            "dev.flutter.pigeon.zeta_aztec_editor.AztecFlutterApi.onAztecFileDeleted$separatedMessageChannelSuffix"
        val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
        channel.send(listOf(filePathArg)) {
            if (it is List<*>) {
                if (it.size > 1) {
    ...
    fun onAztecHtmlChanged(dataArg: String, callback: (Result<Unit>) -> Unit) {
        val separatedMessageChannelSuffix =
            if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
        val channelName =
            "dev.flutter.pigeon.zeta_aztec_editor.AztecFlutterApi.onAztecHtmlChanged$separatedMessageChannelSuffix"
        val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
